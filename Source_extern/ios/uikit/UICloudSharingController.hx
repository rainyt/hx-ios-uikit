...
extern class UICloudSharingController extends UIViewController{
	...
	overload public function initWithNibNameBundle(nibNameOrNil:NSString, bundle:NSBundle):UICloudSharingController;

	@:native("initWithCoder")
	overload public function initWithCoder(coder:NSCoder):UICloudSharingController;

	/* Use this initializer when you want to share a set of CKRecords but haven't yet saved a CKShare.  The preparation handler is called when it is time to save the share to the server.  After ensuring the share and record have been saved to the server, invoke the preparationCompletionHandler  with either the resulting CKShare, or an NSError if saving failed.  */
	...
	overload public function initWithShareContainer(share:Dynamic, container:Dynamic):UICloudSharingController;
	...
	/* Returns an activity item source for use with UIActivityViewController.  If the activity is selected, delegate methods will be called for the original instance of  the sharing controller.  */
