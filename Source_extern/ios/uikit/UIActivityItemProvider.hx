...
extern class UIActivityItemProvider{
	...
	overload extern inline public function activityViewController(activityViewController:UIActivityViewController, itemForActivityType:nullableUIActivityType, called:to, fetch:data, after:an, activity:is, selected.:you, can:return):nullable id;

	@:native("activityViewController:subjectForActivityType://:activity:a:field.")
	overload extern inline public function activityViewController(activityViewController:UIActivityViewController, subjectForActivityType:nullableUIActivityType, //:if, activity:supports, a:Subject, field.:iOS):NSString *;

	@:native("activityViewController:dataTypeIdentifierForActivityType://:for:if:is:NSData.:7.0.:be:with:activity:then")
	overload extern inline public function activityViewController(activityViewController:UIActivityViewController, dataTypeIdentifierForActivityType:nullableUIActivityType, //:UTI, for:item, if:it, is:an, NSData.:iOS, 7.0.:will, be:called, with:nil, activity:and, then:selected):NSString *;

	@:native("activityViewController:thumbnailImageForActivityType:suggestedSize://:activity:preview")
	overload extern inline public function activityViewController(activityViewController:UIActivityViewController, thumbnailImageForActivityType:nullableUIActivityType, suggestedSize:CGSize, //:if, activity:supports, preview:image.):nullable UIImage *;
