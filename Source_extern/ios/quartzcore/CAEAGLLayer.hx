...
import ios.quartzcore.CAEAGLLayer;
...
extern class CAEAGLLayer extends CALayer
{
	...
	/* When false (the default value) changes to the layer's render buffer  * appear on-screen asynchronously to normal layer updates. When true,  * changes to the GLES content are sent to the screen via the standard  * CATransaction mechanisms. */
	...
	overload public function initWithLayer(layer:Dynamic):CAEAGLLayer;
