...
extern class NSSimpleCString extends NSString{
	...
	overload public static function pathWithComponents(components:Dynamic):NSSimpleCString;
	...
	overload public static function localizedNameOfStringEncoding(encoding:Dynamic):NSSimpleCString;
	...
	overload public function initWithCharactersNoCopyLengthFreeWhenDone(characters:Dynamic, length:Int, freeWhenDone:Bool):NSSimpleCString;

	@:native("initWithCharactersNoCopy:length:deallocator")
	overload public function initWithCharactersNoCopyLengthDeallocator(chars:Dynamic, length:Int, deallocator:Dynamic):NSSimpleCString;

	@:native("initWithCharacters:length")
	overload public function initWithCharactersLength(characters:Dynamic, length:Int):NSSimpleCString;

	@:native("initWithUTF8String")
	overload public function initWithUTF8String(nullTerminatedCString:Dynamic):NSSimpleCString;

	@:native("initWithString")
	overload public function initWithString(aString:NSString):NSSimpleCString;

	@:native("initWithFormat:...")
	overload public function initWithFormat...(format,:NSString, ...:Dynamic):NSSimpleCString;

	@:native("initWithFormat:arguments")
	overload public function initWithFormatArguments(format:NSString, arguments:Dynamic):NSSimpleCString;

	@:native("initWithFormat:locale:...")
	overload public function initWithFormatLocale...(format:NSString, locale:Dynamic, ...:Dynamic):NSSimpleCString;

	@:native("initWithFormat:locale:arguments")
	overload public function initWithFormatLocaleArguments(format:NSString, locale:Dynamic, arguments:Dynamic):NSSimpleCString;

	@:native("initWithData:encoding")
	overload public function initWithDataEncoding(data:NSData, encoding:Dynamic):NSSimpleCString;

	@:native("initWithBytes:length:encoding")
	overload public function initWithBytesLengthEncoding(bytes:Dynamic, length:Int, encoding:Dynamic):NSSimpleCString;

	@:native("initWithBytesNoCopy:length:encoding:freeWhenDone")
	overload public function initWithBytesNoCopyLengthEncodingFreeWhenDone(bytes:Void, length:Int, encoding:Dynamic, freeWhenDone:Bool):NSSimpleCString;

	@:native("initWithBytesNoCopy:length:encoding:deallocator")
	overload public function initWithBytesNoCopyLengthEncodingDeallocator(bytes:Void, length:Int, encoding:Dynamic, deallocator:Dynamic):NSSimpleCString;

	@:native("string")
	overload public static function string():NSSimpleCString;

	@:native("stringWithString")
	overload public static function stringWithString(string:NSString):NSSimpleCString;

	@:native("stringWithCharacters:length")
	overload public static function stringWithCharactersLength(characters:Dynamic, length:Int):NSSimpleCString;

	@:native("stringWithUTF8String")
	overload public static function stringWithUTF8String(nullTerminatedCString:Dynamic):NSSimpleCString;

	@:native("stringWithFormat:...")
	overload public static function stringWithFormat...(format,:NSString, ...:Dynamic):NSSimpleCString;

	@:native("localizedStringWithFormat:...")
	overload public static function localizedStringWithFormat...(format,:NSString, ...:Dynamic):NSSimpleCString;

	@:native("initWithCString:encoding")
	overload public function initWithCStringEncoding(nullTerminatedCString:Dynamic, encoding:Dynamic):NSSimpleCString;

	@:native("stringWithCString:encoding")
	overload public static function stringWithCStringEncoding(cString:Dynamic, encoding:Dynamic):NSSimpleCString;

	@:native("initWithContentsOfURL:encoding:error")
	overload public function initWithContentsOfURLEncodingError(url:NSURL, encoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("initWithContentsOfFile:encoding:error")
	overload public function initWithContentsOfFileEncodingError(path:NSString, encoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("stringWithContentsOfURL:encoding:error")
	overload public static function stringWithContentsOfURLEncodingError(url:NSURL, encoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("stringWithContentsOfFile:encoding:error")
	overload public static function stringWithContentsOfFileEncodingError(path:NSString, encoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("initWithContentsOfURL:usedEncoding:error")
	overload public function initWithContentsOfURLUsedEncodingError(url:NSURL, usedEncoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("initWithContentsOfFile:usedEncoding:error")
	overload public function initWithContentsOfFileUsedEncodingError(path:NSString, usedEncoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("stringWithContentsOfURL:usedEncoding:error")
	overload public static function stringWithContentsOfURLUsedEncodingError(url:NSURL, usedEncoding:Dynamic, error:NSError):NSSimpleCString;

	@:native("stringWithContentsOfFile:usedEncoding:error")
	overload public static function stringWithContentsOfFileUsedEncodingError(path:NSString, usedEncoding:Dynamic, error:NSError):NSSimpleCString;
