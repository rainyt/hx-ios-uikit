            ...
            if (((value.find("@property") if ((startIndex is None)) else HxString.indexOfImpl(value,"@property",startIndex))) == 0):
                ...
                if tmp:
                    ...
                    if ((func is not None) and (not (func.name in self._propertys.h))):
                        haxe_Log.trace("写入：",_hx_AnonObject({'fileName': "extern_tools_source/ExternBaseClass.hx", 'lineNumber': 140, 'className': "ExternBaseClass", 'methodName': "new", 'customParams': [func.name]}))
    ...
    def toHaxeFile(self):
        ...
        while (_g_current < len(_g_array)):
            ...
            if ((value.type == "property") and self.hasFuncExtendsOrAttr(value)):
                haxe = (("null" if haxe is None else haxe) + HxOverrides.stringOrNull(((("//" + HxOverrides.stringOrNull(value.name)) + "\n"))))
                continue
            _g = value.type
            _hx_local_12 = len(_g)
            if (_hx_local_12 == 4):
    ...
    def parsingFramework(indir,out):
        ...
        while (_g_current < len(_g_array)):
            ...
            if sys_FileSystem.isDirectory(((("null" if indir is None else indir) + "/") + ("null" if value is None else value))):
                ...
                startIndex = None
                if (((value.find("NSURL.h") if ((startIndex is None)) else HxString.indexOfImpl(value,"NSURL.h",startIndex))) != -1):
...
class ObjcFun:
    ...
    def parsing(typedefs,className,line):
        ...
        if (((line.find("API_DEPRECATED") if ((startIndex is None)) else HxString.indexOfImpl(line,"API_DEPRECATED",startIndex))) != -1):
            return None
        haxe_Log.trace("----------------------",_hx_AnonObject({'fileName': "extern_tools_source/ObjcFun.hx", 'lineNumber': 10, 'className': "ObjcFun", 'methodName': "parsing"}))
        haxe_Log.trace("line = ",_hx_AnonObject({'fileName': "extern_tools_source/ObjcFun.hx", 'lineNumber': 11, 'className': "ObjcFun", 'methodName': "parsing", 'customParams': [line]}))
        startIndex1 = None
        _hx_len = None
        if (startIndex1 is None):
        ...
        while (_g < _g1):
            ...
            if (char != " "):
                ...
                break
        haxe_Log.trace("funcname=",_hx_AnonObject({'fileName': "extern_tools_source/ObjcFun.hx", 'lineNumber': 24, 'className': "ObjcFun", 'methodName': "parsing", 'customParams': [funcName]}))
        args = None
        _g = 0
        _g1 = len(funcName)
        while (_g < _g1):
        ...
        haxe_Log.trace("最终方法：",_hx_AnonObject({'fileName': "extern_tools_source/ObjcFun.hx", 'lineNumber': 43, 'className': "ObjcFun", 'methodName': "parsing", 'customParams': [isStatic, fname]}))
