    ...
    def parsingFramework(indir,out):
        ...
        while (_g_current < len(_g_array)):
            ...
            if sys_FileSystem.isDirectory(((("null" if indir is None else indir) + "/") + ("null" if value is None else value))):
                ...
                startIndex = None
                if (((value.find("NSLayoutConstraint.h") if ((startIndex is None)) else HxString.indexOfImpl(value,"NSLayoutConstraint.h",startIndex))) != -1):
    ...
    def parsing(typedefs,className,line):
        ...
        while (_g < _g1):
            ...
            if (isRead and ((skip <= 0))):
                ...
                skip = (skip - 1)
        haxe_Log.trace("过滤前",_hx_AnonObject({'fileName': "extern_tools_source/ObjcProperty.hx", 'lineNumber': 74, 'className': "ObjcProperty", 'methodName': "parsing", 'customParams': [p]}))
        def _hx_local_7(f):
            p = None
            p1 = None
            p2 = None
        ...
        haxe_Log.trace(p,_hx_AnonObject({'fileName': "extern_tools_source/ObjcProperty.hx", 'lineNumber': 76, 'className': "ObjcProperty", 'methodName': "parsing"}))
