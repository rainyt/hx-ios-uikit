...
class ExternBaseClass:
    ...
    def __init__(self,_hdata,hextern,defcall):
        ...
        if (self.extendClassName is not None):
            haxe_Log.trace("extendClassName = ",_hx_AnonObject({'fileName': "extern_tools_source/ExternBaseClass.hx", 'lineNumber': 79, 'className': "ExternBaseClass", 'methodName': "new", 'customParams': [self.extendClassName]}))
            _this = self.extendClassName
            startIndex = None
            if (((_this.find("<") if ((startIndex is None)) else HxString.indexOfImpl(_this,"<",startIndex))) != -1):
                _this = self.extendClassName
        ...
        if ((self.extendClassName is not None) and ((self.extendClassName == "NSObject"))):
            self.extendClassName = None
        if (pclassName == ""):
            self.className = None
            return
